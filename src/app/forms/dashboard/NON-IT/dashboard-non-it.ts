...
@Component({
  selector: 'dashboard-non-it',
  styleUrls: ['./dashboard-non-it.scss'],
  templateUrl: './dashboard-non-it.html',
  providers: [InventoryHttpService]
})
export class DashboardNONIT implements OnInit {
...
private updateData(inventoryId: number) {
  ...
  this.assetHttpService.totalNONIT(inventoryId).subscribe( (res) => {
...
private updateSubTypeInUseData() {
  ...
  this.assetHttpService.auditSubtypeNONITInUseChart().subscribe( (res) => {
...
private updateSubTypeInStockData() {
  ...
  this.assetHttpService.auditSubtypeNONITInStockChart().subscribe( (res) => {
