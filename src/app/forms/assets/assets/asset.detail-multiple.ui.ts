...
import { AssetMemoryService } from 'app/services/memory/asset.memory.service';
import { AssetList } from './asset.list';
import { AssetSaveMultiple } from 'app/model/api/assets/asset-save-multiple';


@Component({
    selector: 'asset-detail-multiple-ui',
    templateUrl: 'asset.detail-multiple.ui.html',
    styleUrls: ['asset.detail-multiple.ui.scss'],
    providers: [
...
export class AssetDetailMultipleUI  {
    ...
    private asset: AssetSaveMultiple = new AssetSaveMultiple();
    private entityIds: Array<number> = new Array<number>();
    ...
    ngAfterViewInit() {
        ...
        this.assetMemoryService.get(null, null, '', '', new Array<Param>()).subscribe((assets) => {
            ...
            this.selectedAssets.forEach(element => {
                this.entityIds.push(element.id);
            });
            this.refreshAssets();
        });
    ...
    private saveAsset() {
        this.asset.assetIds = new Array<number>();
        this.selectedAssets.forEach(asset => {
            this.asset.assetIds.push(asset.id);
        });
        this.isSaved = false;
        ...
        if (this.asset.id > 0) {
             this.assetHttpService.updateAssetMultiple(this.asset)
            .subscribe(() => {
