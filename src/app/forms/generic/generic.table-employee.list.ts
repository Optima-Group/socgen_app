...
export class GenericTableEmployeeList<T extends IEntity<V>, V> extends GenericList<T, V> {

    protected tableItems: Array<TableItemEmployee<T>> = new Array<TableItemEmployee<T>>();
    ...
    protected doCustomProcessing() {
        ...
        if (this.items != null) {
            this.items.forEach((item) => {
                ...
                this.tableItems.push(new TableItemEmployee<T>(item, selected, false));
            });
        }
    }
    ...
    updateBullsPlayer(index: number) {
        // this.tableItems = this.tableItems.filter((item) => item.selected)
    ...
    private selectAll() {
        this.tableItems.forEach((tItem: TableItemEmployee<T>) => {
            if (!tItem.selected) {
